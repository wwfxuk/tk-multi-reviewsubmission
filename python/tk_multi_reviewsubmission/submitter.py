...
class Submitter(object):
    ...
    def submit_version(self, path_to_frames, path_to_movie, thumbnail_path,
                       sg_publishes, sg_task, comment, store_on_disk,
                       first_frame, last_frame, upload_to_shotgun):
        """
        ...
        if sgtk.util.get_published_file_entity_type(self.__app.sgtk) == "PublishedFile":
            ...
            if len(sg_publishes) > 0:
                if len(sg_publishes) > 1:
                    self.__app.log_warning("Only the first publish of %d can "
                                           "be registered for the new version!"
                                           % len(sg_publishes))
...
class UploaderThread(QtCore.QThread):
    ...
    def run(self):
        ...
        if upload_error or not self._upload_to_shotgun:
            try:
                self._app.sgtk.shotgun.upload_thumbnail("Version",
                                                        self._version["id"],
                                                        self._thumbnail_path)
            except Exception as e:
                self._errors.append("Thumbnail upload to Shotgun failed: "
                                    "%s" % e)
