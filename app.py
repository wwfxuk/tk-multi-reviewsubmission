...
class MultiReviewSubmissionApp(sgtk.platform.Application):
    ...
    def init_app(self):
        ...
        Note, this app doesn't register any commands at the moment as all it's
        functionality is provided through it's API.
        """
        pass
    ...
    def render_and_submit(self, template, fields, first_frame, last_frame,
        ...
        *** Deprecated ***
        ...
        """
        self.logger.warning("The method 'render_and_submit()' has been "
                            "deprecated as it didn't allow the colorspace of "
                            "the input frames to be specified. Please use "
                            "'render_and_submit_version()' instead.")

        # call new version
        return self.render_and_submit_version(template, fields, first_frame,
                                              last_frame, sg_publishes,
                                              sg_task, comment, thumbnail_path,
                                              progress_cb)

    def render_and_submit_version(self, template, fields, first_frame,
                                  last_frame, sg_publishes, sg_task, comment,
                                  thumbnail_path, progress_cb,
                                  color_space=None, *args, **kwargs):
        """
        Main application entry point to be called by other applications/hooks.

        :param template:        The template defining the path where frames
                                should be found.
        :param fields:          Dictionary of fields to be used to fill out
                                the template with.
        :param first_frame:     The first frame of the sequence of frames.
        :param last_frame:      The last frame of the sequence of frames.
        :param sg_publishes:    A list of shotgun published file objects to
                                link the publish against.
        :param sg_task:         A Shotgun task object to link against.
                                Can be None.
        :param comment:         A description to add to the Version in Shotgun.
        :param thumbnail_path:  The path to a thumbnail to use for the version
                                when the movie isn't being uploaded to Shotgun
        ...
        :returns:               The Version Shotgun entity dictionary that was
                                created.
        """
        ...
        if not upload_to_shotgun and not store_on_disk:
            self.logger.warning("App is not configured to store images on "
                                "disk nor upload to shotgun! (Nothing done)")
            return None
        ...
        # Tweak fields so that we'll be getting nuke formated sequence markers
        # (%03d, %04d etc):
        for key in template.keys.values():
            if isinstance(key, sgtk.templatekey.SequenceKey):
                fields[key.name] = "FORMAT: %d"
