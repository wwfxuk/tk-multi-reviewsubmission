...
class CodecSettings(HookBaseClass):
    ...
    def get_quicktime_settings(self, **kwargs):
        ...
        Returns a dictionary of settings to be used for the Write Node that
        generates the Quicktime in Nuke.
        """
        settings = {}
        if sys.platform in ["darwin", "win32"]:
            settings["file_type"] = "mov"
            if nuke.NUKE_VERSION_MAJOR >= 9:
                # Nuke 9.0v1 changed the codec knob name to meta_codec and
                # added an encoder knob (which defaults to the new
                # mov64 encoder/decoder).
            ...
            if nuke.NUKE_VERSION_MAJOR >= 9:
                # Nuke 9.0v1 removed ffmpeg, replaced it with the mov64 writer
